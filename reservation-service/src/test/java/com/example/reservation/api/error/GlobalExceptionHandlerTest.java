...
class GlobalExceptionHandlerTest {
    ...
    void handleApiExceptionUsesStatusCodeDetailsAndTraceHeader() {
        ...
        var exception = new NotFoundException(ErrorCode.RESERVATION_NOT_FOUND, "Missing reservation.", Map.of("reservationId", "r1"));

        var response = handler.handleApiException(exception, request);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
        assertThat(response.getBody().data()).isNull();
        assertThat(response.getBody().error().code()).isEqualTo("RESERVATION_NOT_FOUND");
        assertThat(response.getBody().error().details()).containsEntry("reservationId", "r1");
    ...
    void handleValidationCollectsFieldErrorsAndFallsBackToRequestId() {
        ...
        when(bindingResult.getFieldErrors()).thenReturn(List.of(new FieldError("request", "orderId", "must not be blank")));
        ...
        assertThat(response.getBody().error().details()).containsEntry("orderId", "must not be blank");
    ...
    void handleDomainExceptionReturnsValidationError() {
        ...
        assertThat(response.getBody().data()).isNull();
        assertThat(response.getBody().error().code()).isEqualTo("VALIDATION_ERROR");
        assertThat(response.getBody().error().message()).isEqualTo("orderId is required.");
